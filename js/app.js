...
function initApp() {
    ...
    setupAdminTools(); // Initialize Admin Tools (Backup/Delete)
...
function setupAdminTools() {
    ...
    if (btnDelete) {
        btnDelete.addEventListener('click', async () => {
            ...
            const history = DataManager.getHistory();
            const candidates = history.filter(l => {
                const t = new Date(l.timestamp);
                return t >= startDate && t <= endDate;
            });

            if (candidates.length === 0) {
                alert("No se encontraron registros en ese rango (en la caché actual).");
                return;
            }

            if (confirm(`⚠️ PELIGRO ⚠️\n\nSe van a ELIMINAR PERMANENTEMENTE ${candidates.length} registros del historial.\nRango: ${startStr} a ${endStr}\n\n¿Estás seguro? Esta acción no se puede deshacer.`)) {
                ...
                try {
                    ...
                    const res = await DataManager.deleteHistoryLogs(ids);
