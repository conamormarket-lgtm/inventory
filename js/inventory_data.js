...
const COLLECTION_INVENTORY = 'inventory_deprecated_v1'; // Prevent accidental reads of legacy 'inventory'
...
export const DataManager = {
    initListener(onUpdate) {
        // ... (inventory stats listener unchanged)
        ...
        inventoryUnsubscribe = onSnapshot(statsRef, (docSnap) => {
            ...
            if (docSnap.exists()) {
                ...
                if (data.items && Array.isArray(data.items)) {
                    localInventoryCache = data.items;
                }
            }
        ...
        // History Listener
        // Optimized: Limit 50
        const qHistory = query(collection(db, COLLECTION_HISTORY), orderBy('timestamp', 'desc'), limit(50));
        historyUnsubscribe = onSnapshot(qHistory, (snapshot) => {
            localHistoryCache = [];
        ...
        // Users Listener (Limited)
        const qUsers = query(collection(db, COLLECTION_USERS), limit(50));
        usersUnsubscribe = onSnapshot(qUsers, (snapshot) => {
            if (snapshot.empty) {
                // this.seedUsers(); // Disable auto-seed to prevent writes on read-check
            } else {
                localUsersCache = [];
                snapshot.forEach(doc => {
