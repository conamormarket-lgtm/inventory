...
const SIZE_ORDER = ['2', '4', '6', '8', '10', '12', '14', '16', 'XS', 'S', 'M', 'L', 'XL', 'XXL'];
    ...
    DataManager.initListener((update) => {
        if (update.type === 'inventory') {
            ...
            if (currentPanel && currentPanel.id === 'stats-panel') {
                renderChart();
                // Refresh active subview (Color, Size, etc.)
                const activeSubBtn = document.querySelector('.subnav-btn.active');
                if (activeSubBtn) renderStatsSubView(activeSubBtn.dataset.view);
            }
        }
...
function showDashboard() {
    ...
    // Default to Movements panel whenever entering dashboard
    switchPanel('movements-panel');
}
...
function switchPanel(panelName) {
    ...
    if (panelName === 'reports-panel') {
        // renderReports(); // DISABLED (Missing Function)
...
// Search Listener
// Search Listener (Removed Stock Search)
...
// SIZE_ORDER moved to top

function setupCustomSelect(selectId, dataArray, displayField = null, customSort = null) {
    ...
    } else if (selectId.includes('mov-size') || selectId.includes('dd-filter-size')) {
        // Auto-detect Size Sort if not explicit
        sortedData.sort((a, b) => {
            const valA = (typeof a === 'object' && a[displayField]) ? a[displayField] : a;
            const valB = (typeof b === 'object' && b[displayField]) ? b[displayField] : b;

            // Normalize for search
            const indexA = SIZE_ORDER.indexOf(String(valA).toUpperCase());
            const indexB = SIZE_ORDER.indexOf(String(valB).toUpperCase());

            // If both found in predefined list
            if (indexA !== -1 && indexB !== -1) return indexA - indexB; // Ascending index (XXL is 0, XS is 5) -> "Mayor a Menor"? 
            // Wait, user said "Mayor a Menor" => Largest first.
            // My list is XXL, XL, L, M... so index 0 is Largest.
            // Sort A-B: 0 - 5 = -5 (Negative), so A comes first.
            // So this sorts XXL, then XL... Correct.
    ...
    // Default value logic: if "Todos/as" exists, prefer it? Or just first.
    // If "Todos" is in list, it usually gets sorted to bottom by Alpha, or manual array pushes.
    // Let's rely on sortedData[0] for now.
    trigger.innerHTML = `<span>${sortedData[0] ? ((typeof sortedData[0] === 'object' ? sortedData[0][displayField] : sortedData[0])) : 'Seleccionar'}</span>`;
    ...
    function openSelect() {
        ...
        searchInput.value = '';
        optionsList.querySelectorAll('.select-option').forEach(opt => opt.style.display = 'block');
...
function setupMovementForm() {
    ...
    form.addEventListener('submit', (e) => {
        ...
        // Pre-Validation (Local Check for Speed)
        if (currentType === 'exit') {
            const inventory = DataManager.getInventory();
            // Find Item
            const item = inventory.find(i =>
                i.type === details.type &&
                i.color === details.color &&
                i.size === details.size
            );
        ...
        submitBtn.disabled = true;
        submitBtn.innerHTML = '<i class="ph ph-check"></i> Guardado';
        submitBtn.style.background = 'var(--success)';

        // Background Process (Fire & Forget)
        DataManager.addMovement(
            ...
            if (!result.success) {
                alert(`⚠️ Error de Sincronización: ${result.error}\nPor favor recarga la página.`);
            }
            // Success is silent (UI already updated)
        }).catch(err => {
        ...
        setTimeout(() => {
            ...
            // Restore button with animation or delay
            setTimeout(() => {
                submitBtn.disabled = false;
                submitBtn.innerHTML = originalBtnText;
                submitBtn.style.background = '';
                // Restore search selects? They observe native select.
                // Resetting form resets native selects to index 0.
                // We might need to trigger change events to update Custom Selects UI.
                document.getElementById('mov-garment').dispatchEvent(new Event('change'));
                document.getElementById('mov-color').dispatchEvent(new Event('change'));
                document.getElementById('mov-size').dispatchEvent(new Event('change'));
            }, 1000);
        }, 500);
    });
...
function renderStatsMatrix(container) {
    ...
    let usedSizes = (metadata && metadata.sizes && metadata.sizes.length > 0) ? [...metadata.sizes] : [...SIZES];
    const usedGarments = (metadata && metadata.garments && metadata.garments.length > 0) ? metadata.garments : GARMENT_TYPES;

    // SORT SIZES (Smallest to Largest) using SIZE_ORDER
    usedSizes.sort((a, b) => {
        const indexA = SIZE_ORDER.indexOf(String(a).toUpperCase());
        const indexB = SIZE_ORDER.indexOf(String(b).toUpperCase());
        // If both in list, simple index sort
        if (indexA !== -1 && indexB !== -1) return indexA - indexB;
        // If one missing, put at end (custom/new sizes)
        if (indexA !== -1) return -1;
        if (indexB !== -1) return 1;
        // If both missing, alpha sort
...
window.renderDrillDownDashboard = function (type) {
    ...
    // Initialize Components (Ensure no duplicates)
...
async function generateReport() {
    ...
    // --- GROUPING LOGIC ---
    const groups = {}; // { 'Polera': { total: 0, variants: { 'Negro - M': 10 } } }

    filtered.forEach(log => {
        let type = 'Desconocido';
        let detailKey = 'N/A';
        let qty = log.quantity || 1;

        // Try Metadata first (Robust)
        if (log.metadata) {
            type = log.metadata.type || 'Otros';
            // Detail: Color - Size
            if (log.metadata.color && log.metadata.size) {
                detailKey = `${log.metadata.color} - ${log.metadata.size}`;
            } else {
                detailKey = log.details || 'General';
            }
            if (log.metadata.quantity) qty = log.metadata.quantity;
        } else {
            // Fallback: Parse String
            // Expected: "Polera - Negro - Talla M (Cant: 5)"
            const parts = log.details.split(' - ');
            if (parts.length >= 1) type = parts[0].trim();

            // Try to extract quantity if not on root
            // Qty logic was already handled in previous loop example but let's redo inline
            const match = log.details.match(/\(Cant:\s*(\d+)\)/);
            if (match && match[1]) qty = parseInt(match[1]);

            if (parts.length >= 3) {
                // Try to reconstruct "Color - Size"
                // Remove known suffixes like "(Cant: 1)"
                const rawDetail = parts.slice(1).join(' - ').replace(/\(Cant:.*?\)/, '').trim();
                detailKey = rawDetail.replace('Talla ', ''); // Clean "Talla "
            } else {
                detailKey = log.details;
            }
        }
        ...
        groups[type].total += qty;

        if (!groups[type].variants[detailKey]) groups[type].variants[detailKey] = 0;
        groups[type].variants[detailKey] += qty;
    });

    // --- RENDER GROUPED TABLE ---
    // Header
    let table = `
    <div style="padding:15px; font-size:1.1rem; font-weight:bold; border-bottom:1px solid var(--border-color); color:var(--text-main); display:flex; justify-content:space-between; align-items:center;">
        <span><i class="ph ph-files"></i> Reporte Generado (${Object.keys(groups).length} Categorías)</span>
        <span style="font-size:0.9rem; color:var(--text-muted); font-weight:400;">${startDate.toLocaleDateString()} - ${endDate.toLocaleDateString()}</span>
    </div>
    <div style="overflow-x:auto;">
        <table class="report-grouped-table">
            <thead>
                <tr>
                    <th style="padding-left:30px;">CATEGORÍA</th>
                    <th>DETALLE (COLOR / TALLA)</th>
    ...
    // Body
    // Sort Categories by Total Quantity Desc
    const sortedCategories = Object.keys(groups).sort((a, b) => groups[b].total - groups[a].total);

    sortedCategories.forEach(cat => {
        const data = groups[cat];
        // Sort Variants by Qty Desc
        const sortedVariants = Object.entries(data.variants).sort((a, b) => b[1] - a[1]);

        // Logic for "Top N" items
        const TOP_N = 6;
        const topItems = sortedVariants.slice(0, TOP_N);
        const remainder = sortedVariants.slice(TOP_N);
        const remainderCount = remainder.reduce((sum, item) => sum + item[1], 0); // Sum of quantities (or count of items? image says "+131 más", usually means item types)
        // Image "+131 más" likely means more rows/variants.
        ...
        if (remainder.length > 0) {
            detailsHtml += `
                <div class="report-detail-item" style="color:var(--primary); font-style:italic; padding-top:8px;">
                    <span>(+${remainder.length} variantes más)</span>
                </div>
            `;
        }

        // Color for Total: Red (Danger) or Green (Success) based on Action?
        // User requested Red in image context? Image shows Red totals. Let's stick to theme (Green for Entry, Red for Exit) or Neutral.
        // Image shows "TOTAL 1044" in Red. Text is red-ish/orange.
        const totalColor = currentReportType === 'Entrada' ? 'var(--success)' : 'var(--danger)';

        table += `
            <tr class="report-item-row">
                <td class="report-category-cell" style="padding-left:30px;">
                    <div style="font-size:1.1rem;">${cat}</div>
                </td>
                <td class="report-detail-cell">
                    ${detailsHtml}
                </td>
                <td class="report-total-cell" style="color:${totalColor}; padding-right:30px;">
                    ${data.total}
                </td>
            </tr>
        `;
...
function setupAdminTools() {
    ...
    // Add Migration Button (Optimization) - Re-enabled
    if (adminZone && !document.getElementById('btn-migration-tool')) {
        // Find the grid container definition inside admin zone
        const gridContainer = adminZone.querySelector('div[style*="display: grid"]');

        if (gridContainer) {
            const migCard = document.createElement('div');
            migCard.className = 'admin-card';
            migCard.style.border = '1px dashed #8b5cf6';
            migCard.innerHTML = `
                <h4><i class="ph ph-lightning"></i> Optimización</h4>
                <p style="font-size: 0.85rem; color: var(--text-muted); margin-bottom: 15px;">
                    Migrar datos a nueva estructura.
                </p>
                <button id="btn-migration-tool" class="btn btn-block" style="background:#8b5cf6;">
                    <i class="ph ph-database"></i> RESCATAR DATOS (MIGRAR)
                </button>
            `;

            migCard.querySelector('button').onclick = async () => {
                if (!confirm("¿Ejecutar migración? Esto reparará la vista optimizada.")) return;
                const { runMigrationToSingleDoc } = await import('./migration_v2.js');
                await runMigrationToSingleDoc();
            };

            gridContainer.appendChild(migCard);
        }
    }
    ...
    // --- Import Stock (CSV) ---
    const btnImportStock = document.getElementById('btn-import-stock');
    const fileImportStock = document.getElementById('file-import-stock');
    const chkDryRun = document.getElementById('chk-import-dryrun');

    if (btnImportStock && fileImportStock) {
        btnImportStock.addEventListener('click', () => fileImportStock.click());

        fileImportStock.addEventListener('change', (e) => {
            const file = e.target.files[0];
            if (!file) return;

            const isDryRun = chkDryRun ? chkDryRun.checked : true; // Default to safety
            const reader = new FileReader();

            reader.onload = async (evt) => {
                const text = evt.target.result;
                btnImportStock.disabled = true;
                btnImportStock.innerHTML = '<i class="ph ph-spinner ph-spin"></i> Procesando...';

                try {
                    // Call DataManager
                    const res = await DataManager.importStockFromCSV(text, isDryRun);

                    if (res.success) {
                        // Support both new totalUnits and legacy totalQuantity
                        const total = res.totalUnits !== undefined ? res.totalUnits : (res.totalQuantity || 0);

                        if (isDryRun) {
                            // Show Custom Preview Modal
                            const previewData = res.preview || [];
                            // If legacy Matrix mode, preview might be different or empty? 
                            // Matrix mode returns summaryByType, we can use that if preview is empty.

                            let htmlContent = '';

                            if (previewData.length > 0) {
                                // Aggregate Grouping: Type + Color
                                const groups = {};
                                previewData.forEach(item => {
                                    const key = `${item.type}__${item.color}`;
                                    if (!groups[key]) {
                                        groups[key] = { type: item.type, color: item.color, quantity: 0 };
                                    }
                                    groups[key].quantity += item.quantity;
                                });

                                // Convert back to array and Sort: Type > Color
                                const aggregatedDocs = Object.values(groups).sort((a, b) => {
                                ...
                                htmlContent = `
                                    <table style="width:100%; border-collapse:collapse; margin-top:10px; font-size:0.9rem;">
                                        ...
                                        <tbody>
                                            ${aggregatedDocs.map(item => `
                                                <tr style="border-bottom:1px solid rgba(255,255,255,0.05);">
                                                    <td style="padding:6px 8px;">${item.type}</td>
                                                    <td style="padding:6px 8px;">${item.color}</td>
                                                    <td style="padding:6px 8px; text-align:right; font-weight:bold; color:var(--success);">${item.quantity}</td>
                                                </tr>
                                            `).join('')}
                                        </tbody>
                                    </table>
                                `;
                            } else if (res.summaryByType) {
                                // Fallback Matrix Summary
                                htmlContent = `
                                    <ul style="margin-top:10px; padding-left:20px;">
                                        ${Object.entries(res.summaryByType).map(([k, v]) => `<li>${k}: ${v} un.</li>`).join('')}
                                    </ul>
                                    <p style="font-size:0.8rem; color:var(--text-muted); margin-top:10px;">(Vista detallada fila por fila no disponible en modo Matriz)</p>
                                `;
                            }
                            ...
                            modal.style.cssText = `
                                ...
                                padding: 25px; border-radius: 12px; display: flex; flex-direction: column;
                                box-shadow: 0 10px 30px rgba(0,0,0,0.5);
                            `;

                            modal.innerHTML = `
                                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                                    <h3 style="margin:0; color:var(--accent);"><i class="ph ph-eye"></i> Vista Previa de Importación</h3>
                                    <button id="btn-close-preview" style="background:none; border:none; color:var(--text-muted); cursor:pointer; font-size:1.5rem;">&times;</button>
                                </div>
                                <div style="display:flex; justify-content:space-between; margin-bottom:15px; background:rgba(255,255,255,0.05); padding:10px; border-radius:8px;">
                                    <div><strong>Items Detectados:</strong> ${res.count}</div>
                                    <div style="color:var(--success);"><strong>Total Prendas:</strong> ${total}</div>
                                </div>
                                <div style="overflow-y:auto; flex:1; border:1px solid var(--border-color); border-radius:6px; padding:10px;">
                                    ${htmlContent || '<p>No hay datos detallados disponibles.</p>'}
                                </div>
                                <div style="margin-top:20px; text-align:right;">
                                    <button id="btn-ok-preview" class="btn" style="background:var(--primary);">Entendido</button>
                                </div>
                            `;
                            ...
                            modal.querySelector('#btn-ok-preview').onclick = close;

                        } else {
                            alert(`🟢 IMPORTACIÓN COMPLETADA\n\nSe han actualizado/creado ${res.count} items.\nTotal de Prendas: ${total}`);
                            // Success Logic
                            // (Migration button removed from here)
                        }
                    } else {
                        alert("⚠️ Error en importación:\n" + res.error);
                    }
                } catch (err) {
                    console.error(err);
                    alert("Error crítico: " + err.message);
                } finally {
                    btnImportStock.disabled = false;
                    btnImportStock.innerHTML = '<i class="ph ph-upload"></i> Cargar CSV de Stock';
                    fileImportStock.value = '';
                }
            };
            reader.readAsText(file);
        });
...
function renderUserList() {
    ...
    // Auto-hide if no users yet (wait for sync)
    if (users.length === 0) {
        container.innerHTML = '<p style="padding:10px; color:var(--text-muted);">Cargando usuarios...</p>';
        return;
    }
